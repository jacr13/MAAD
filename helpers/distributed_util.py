...
def mpi_moments(x, comm=COMM, axis=0, keepdims=False):
    ...
    mean_squared_diff, count1 = mpi_mean_reduce_v(
        squared_diffs,
        axis=axis,
        comm=comm,
        keepdims=True,
...
def sync_weights(model, comm=COMM, msg=""):
    ...
    logger.info(f"[INFO]{msg} workers synced with root")


def sync_data(data, to_all=False, comm=COMM):
    """Aggregates the data into a list and returns it to master"""
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    if to_all:
...
def send_data_to_all(data, comm=MPI.COMM_WORLD):
    """
    Send data from one process (rank 0) to all other processes.
    ...
    Returns:
        data_received: The received data on all processes.
    """
    rank = comm.Get_rank()
    size = comm.Get_size()
    if rank == 0:
        # On rank 0, send data to all other processes
        for dest_rank in range(1, size):
            comm.send(data, dest=dest_rank)
    else:
        # On other ranks, receive data from rank 0
...
def mpi_save_random_states(agent, ckpt_dir, iters_so_far, save=False, comm=COMM):
    comm.Barrier()
    rank = comm.Get_rank()
...
class RunMoms(object):
    ...
    def state_dict(self):
        return {key: value for key, value in self.__dict__.items() if key != "comm"}
