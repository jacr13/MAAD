...
def finish():
    log("Sending final files to wandb.")
    import wandb

    Logger.CURRENT.dir_
    # save log output in wandb since the file already present doesn't save it
    shutil.copy(osp.join(Logger.CURRENT.dir_, "log.txt"), os.path.join(wandb.run.dir, "log.txt"))
    wandb.save(osp.join(Logger.CURRENT.dir_, "log.txt"), base_path=Logger.CURRENT.dir_)
    # save our hyperparameters in wandb in the correct format
    shutil.copy(osp.join(Logger.CURRENT.dir_, "hyperparameters.yaml"), os.path.join(wandb.run.dir, "hyperparameters.yaml"))
    wandb.save(
        osp.join(Logger.CURRENT.dir_, "hyperparameters.yaml"),
        base_path=Logger.CURRENT.dir_,
    )
...
def configure_default_logger():
    ...
    log(
        "configuring default logger for each worker (logging to stdout only by default)",
    )
