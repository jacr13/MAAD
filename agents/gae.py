...
def gae_v2(rollout, gamma, lambda_, rew_key):
    last_r = rollout["next_v"][0]
    vpred_t = np.concatenate([rollout["vs"], np.array([last_r])])
    delta_t = rollout[rew_key] + gamma * vpred_t[1:] - vpred_t[:-1]

    # This formula for the advantage comes from:
    # "Generalized Advantage Estimation": https://arxiv.org/abs/1506.02438
    rollout["advs"] = discount_cumsum(delta_t, gamma * lambda_)
    rollout["td_lam_rets"] = rollout["advs"] + rollout["vs"]
    return rollout


def gae_v2_ev(rollout, gamma, lambda_, rew_key):
    last_r = np.expand_dims(rollout["next_v"][0], axis=-1)
    vpred_t = np.concatenate([rollout["vs"], last_r], axis=1)
    delta_t = rollout[rew_key] + gamma * vpred_t[:, 1:] - vpred_t[:, :-1]
