...
from helpers.math_util import discount_cumsum
from helpers.misc_util import zipsame
from helpers.segment_tree import MinSegmentTree, SumSegmentTree
...
class ReplayBuffer(object):
    def __init__(self, capacity, shapes, device="cpu"):
        self.capacity = capacity
        self.shapes = shapes
        self.device = device
        self.ring_buffers = {
            n: RingBuffer(self.capacity, s, device=self.device)
    ...
    def lookahead(self, transitions, n, gamma, patcher):
        ...
        for idx in transitions["idxs"]:
            # Create indexes of transitions in lookahead of lengths max `n`
            # following sampled one
            ...
            la_discounted_sum_n_rews = discount_cumsum(la_rews, gamma)[0]
    ...
    def latest_entry_idx(self):
        # Since all the functions do exactly the same for every RingBuffer,
        # pick arbitrarily
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def __init__(
        ...
        """
        super(PrioritizedReplayBuffer, self).__init__(capacity, shapes, device=device)
    ...
    def update_priorities(self, idxs, priorities):
        ...
        if self.ranked:
            ...
            idxs = list(_idxs)
            priorities = [1.0 / (rank + 1) for rank in ranks]  # start ranks at 1
        ...
        for idx, priority in zipsame(idxs, priorities):
            ...
            assert (
                0 <= idx < self.num_entries
            ), "no element in buffer associated w/ index"
...
class UnrealReplayBuffer(PrioritizedReplayBuffer):
    ...
    def __init__(self, capacity, shapes, device="cpu", max_priority=1.0):
        ...
        """
        super(UnrealReplayBuffer, self).__init__(
            capacity,
            shapes,
            1.0,
            1,
        ...
        # Create two extra `SumSegmentTree` objects: one for 'bad' transitions,
        # one for 'good' ones
    ...
    def update_priorities(self, idxs, priorities):
        ...
        for idx, priority in zipsame(idxs, priorities):
            ...
            rank = (1.0 / priority) - 1
            thres = floor(0.5 * self.num_entries)
            is_g = rank < thres
